...
function findModelDefinitions(rootDir, sources) {
  ...
  sources.forEach(function (src) {
    if (path.extname(src) === '.json') {
      handleDirectoryPatternExpression(rootDir, src, updateRegistry);
    } else {
      ...
      files
        ...
        .forEach(function (f) {
          ...
          updateRegistry(fullPath, files);
        });
    }
  });
  function updateRegistry(fullPath, files) {
...
function handleDirectoryPatternExpression(rootDir, src, updateRegistry) {
  var moreFiles = glob.sync(src);
  if (moreFiles.length > 0) {
    moreFiles.forEach(function (jsonFile) {
      var fullFilePath = path.resolve(rootDir, jsonFile);
      if (fs.existsSync(fullFilePath)) {
        updateRegistry(fullFilePath, moreFiles);
      } else {
        debug('Skipping unknown module source dir %j', jsonFile);
        return;
      }
    })
  } else {
